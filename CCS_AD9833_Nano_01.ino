...
const float grad_upd = 0.00555556;  // 2  / 360
//const char *str_BL16 = "1234567890123456";
...
int8_t   pos_i=15,pos_o=15,dig_o=0,dig_n=0,s_sel,ila = 2;
uint32_t time_del=10,delay_s=0,off_s=0,off_r=0;
unsigned long time_f,time_l,time_d,time_n;
long     fact=1,fact_o=1,iconst=0,fac_i=0,offs=0;
uint8_t  s_first=0,s_upd=1,digi,s_ms=0,s_s=0,s_stepa=0,s_par=0,s_end=0,s_kHz=0;
...
*/
  ...
  digitalWrite(SWITCH_R, LOW);  // sets the digital pin on // <<<============================ test
    ...
    while(but_p == 0) {          // push-button not pressed
      ...
      if (oldEncPos != EncPos) {
        if (s_first == 0) s_first = 1;
          ...
          if (zoff >= 10) {
            lcd.setCursor(5,1);lcd.print(zoff);
            lcd.setCursor(6,1);lcd.print(".");
            if (offs > 19) zoff = zoff - 20;
            else           zoff = zoff - 10;
            lcd.setCursor(7,1);lcd.print(zoff);   
          }
          else {
            lcd.setCursor(6,1);lcd.print(".");
            lcd.setCursor(7,1);lcd.print(zoff);
          }
          lcd.setCursor(8,1);  lcd.print("V"); 
        }     
        oldEncPos = EncPos;
        delay(100);
      }  // if (old
      ...
      if (oldEncPos != EncPos) {
        if (s_first == 0) s_first = 1;
          ...
          if      (ilt <=  9) {      // 0...500 µs  //  0... 9
            s_ms = 0;  // µs
            lcd.setCursor( 14, 1);  lcd.print("\344s"); 
          ...
          lcd.setCursor( 10, 1);                               lcd.print("    ");
          if      (time_del <      10) {lcd.setCursor( 13, 1); lcd.print(time_del);}
          else if (time_del <     100) {lcd.setCursor( 12, 1); lcd.print(time_del);}
          else if (time_del <    1000) {lcd.setCursor( 11, 1); lcd.print(time_del);}
          else                         {lcd.setCursor( 10, 1); lcd.print(time_del);}     
        }
        oldEncPos = EncPos;
        delay(100);
      }   // if
...
void runSine()  {  // Sine
   ...
   for(grad=0;grad<90;grad++) {       // quarter-Sinewave in Tab sin_t[90]
     ...
     wert = wert*1000.00;
     sinx = (int)(wert+0.5);
     //   Serial.print(grad);Serial.print(" ");Serial.println(sinx);
   ...
   while(but_p == 0) {   // run Sine-wave
     for(grad=0;grad<90;grad++) {  // first 0...90°
       sinx = sin_t[grad];
       if (gainf != 1.0) sinx = sinx * gainf;
       if (sinx > 4095) sinx = 4095;
       if (off_s > 0)   sinx = sinx + off_s;
       if (sinx > 4095) sinx = 4095;
  //     Serial.print(grad);Serial.print(" ");Serial.println(sinx);
       dac.setVoltageA(sinx);  
       dac.updateDAC();
       if (delay_s > 0) {
     ...
     for(grad=90;grad>0;grad--) {  // second 0...90°
       sinx = sin_t[grad];
       if (gainf != 1.0) sinx = sinx * gainf;
       if (sinx > 4095) sinx = 4095;
       if (off_s > 0)   sinx = sinx + off_s;
  ...
  //     Serial.print(grad);Serial.print(" ");Serial.println(sinx);
     ...
     for(grad=0;grad<90;grad++) { // third 0...90° ##### look 2000-sin_t[grad] ##############
       sinx = 2000-sin_t[grad];
       if (gainf != 1.0) sinx = sinx * gainf;
       if (sinx > 4095) sinx = 4095;
       if (off_s > 0) sinx = sinx + off_s;
       if (sinx > 4095) sinx = 4095;
     ...
     for(grad=90;grad>0;grad--) { // fourt 0...90° ##### look 2000-sin_t[grad] ##############
       sinx = 2000-sin_t[grad];
       if (gainf != 1.0) sinx = sinx * gainf;
       if (sinx > 4095) sinx = 4095;
       if (off_s > 0) sinx = sinx + off_s;
...
void runTriangle()  {  // Triangle
   ...
   while(but_p == 0) {
     for(grad=0;grad<180;grad++) { //
       ...
       wert = wert*1000.00;
       if (gainf != 1.0) wert = wert * gainf;
       if (wert > 4095.0) wert = 4095.0;
       if (off_s > 0) wert = wert + (float)off_s;
       if (wert > 4095.0) wert = 4095.0;
       dac.setVoltageA(wert);  
       dac.updateDAC();
       if (delay_s > 0) {
         if (s_ms == 0) delayMicroseconds(delay_s);  // µs
         else           delay(delay_s);              // ms
...
void runSquareWave()  {  // SqareWave
   ...
   while(but_p == 0)  {
     ...
     while (1 == 1)  { //
       ...
       dac.updateDAC();
       if (time_del > 0) {
         if (s_ms == 0) delayMicroseconds(time_del);
         else           delay(time_del);
       } 
     }  // while (1==1
     if (but_p == 1) return;
...
void  ConstCurGraph() {
   ...
   grast:
    ...
    fillBlank(0,0,16); //lcd.setCursor(0,0);     lcd.print("                ");
    lcd.setCursor(0,1);     lcd.print("select Function:");
    ...
    while (but_p == 0) { 
      ...
      if  (ifu > 4) ifu = 0;
      lcd.setCursor(4, 0);
      if      (ifu == 0)  lcd.print("  Sine  ");
      else if (ifu == 1)  lcd.print("Triangle");
      else if (ifu == 2)  lcd.print(" StepUp ");
      else if (ifu == 3)  lcd.print("StepDown");
      else if (ifu == 4)  lcd.print("SquareW.");
      delay(1200);  
    }  // while (but_p == 
   ...
   func_w:    
    ...
    delay(100);
    fillBlank(0,0,4);  //lcd.setCursor(0,0);     lcd.print("    ");
    fillBlank(12,0,4); //lcd.setCursor(12,0);    lcd.print("    ");
      ...
      if (oldEncPos != EncPos) { 
        if (s_first == 0) s_first = 1;
          ...
          if   (oldEncPos < EncPos) {   // 'r'
            fillBlank(0,1,13); //lcd.setCursor(0,1);     lcd.print("             ");
            return;                    //######################################
          }   
          else goto grast;             //##############################
        }   
...
//     ad9.EnableOutput(true);   // Turn ON the output - it defaults to OFF
   ...
   else if (ila == 1) set_Iconst();
   //else if (ila == 2) ConstCurGraph();
   but_p = 0;  
   delay(100);
}
